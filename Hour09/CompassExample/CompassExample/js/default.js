...
(function () {
    ...
    function init() {
        ...
        if (compass) {
            displayReading(compass.getCurrentReading());
            compass.addEventListener("readingchanged", onReadingChanged);
        }
    }

    function onReadingChanged(evt) {
        displayReading(evt.reading);
    }

    function displayReading(reading) {
        var angle = reading.headingMagneticNorth.toFixed(0);

        document.getElementById("angle").textContent = angle;

        document.querySelector('line').attributes["transform"].value =
